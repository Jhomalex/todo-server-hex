...
import { createTasklistMemberInteractor } from '../../core/usesCases';
import { TasklistMemberResponse } from '../TasklistMemberSchema';
import { CreateTasklistMemberInput } from './CreateTasklistMemberInput';

@Resolver()
export class CreateTasklistMemberResolver {
	@Mutation(() => TasklistMemberResponse)
	async createTasklistMember(
		@Arg('data', () => CreateTasklistMemberInput)
		data: CreateTasklistMemberInput,
		@Ctx() ctx: Context
	) {
		try {
			if (!ctx.user) throw new InvalidException('Unauthorized');

			const tasklist = await createTasklistMemberInteractor.run({
